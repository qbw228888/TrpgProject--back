...
@Service
@Slf4j
@ServerEndpoint("/websocket/{userId}/{rid}")
...
//                log.info(messageService.toString());
                if(jsonObject.getString("type").equals("message")){//是用户发送的消息
                    CodeEnum codeEnum = messageService.saveMessage(jsonObject);
                }
                for (Map.Entry<String, WebSocketServer> entry: webSocketMap.entrySet()){
                    if (entry.getKey().equals(this.userId)){
                        continue;
                    }
                    webSocketMap.get(entry.getKey()).sendMessage(JSON.toJSONString(jsonObject));
                }
            }catch (Exception e){
                e.printStackTrace();
            }
